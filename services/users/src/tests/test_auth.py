...
def test_logout_user(test_app, test_database, add_user):
    user = add_user("Matt", "matt@email.com", "123456")
    client = test_app.test_client()
    resp_login = client.post(
        "/auth/login",
    ...
    assert "Access token revoked" in data["msg"]


def test_invalid_logout(test_app, test_database, add_user):
    user = add_user("Matt", "matt@email.com", "123456")
    client = test_app.test_client()
    resp_login = client.post(
        "/auth/login",
